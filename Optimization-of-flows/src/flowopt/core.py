...
def mno_scope_coverage_summary(dataset: RoutingDataset, routes: list[Route]) -> dict[str, Any]:
    ...
    # Eligibility must follow the current dataset task-space (e.g. percentile sweeps),
    # not full-day node metadata copied from the source graph.
    eligible_mno = {
        task.source_node_id
        for task in dataset.tasks
        if (
            dataset.graph.nodes[task.source_node_id].kind == "mno"
            and task.container_type not in SECOND_SHOULDER_CONTAINER_TYPES
        )
    }
